...
| [how-to-handle-drf-spectacular-warnings.md](how-to-handle-drf-spectacular-warnings.md) | Architect (CI / settings) + dev agent (every story adding DRF views or auth classes) — five-rule discipline that prevents the W001/W002 warning cascade in `manage.py check --deploy` | Read at Django project setup; reference rules during stories that add `@api_view`/APIView classes or custom DRF auth. No copy into `_bmad-output/` needed (only for projects using Django + DRF + drf_spectacular). |
