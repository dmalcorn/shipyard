...
export function scoreSynthesisOutput(
  actual: string,
  expected: SynthesisRubric,
): RubricScore {
  return {
    contains: expected.must_contain.every((s) => actual.includes(s)),
    excludes: !expected.must_not_contain.some((s) => actual.includes(s)),
    cites_sources:
      actual.match(/\[interview_\d+\]/g)?.length ?? 0 >= expected.min_citations,
...
export function makeInterviewSession(
