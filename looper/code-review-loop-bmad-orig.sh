...
#
# Compare with: code-review-loop.sh (direct Claude review without BMAD)
#
...
# =============================================================================
# Get ALL stories in an epic (regardless of status)
# =============================================================================
get_all_stories_in_epic() {
    ...
    # Match all stories for the epic: X-N-story-name: status
    grep -E "^\s+${EPIC_NUMBER}-[0-9]+-[^:]+:" "$sprint_file" | sed 's/^\s*//' || true
}
...
review_story() {
    ...
    # Build the BMAD-enforced review prompt
    # NOTE: This prompt FORCES the BMAD workflow - no direct file reads before invoking
    local prompt="YOU MUST USE THE BMAD AGENT WORKFLOW. Do NOT read any files directly.

Step 1: IMMEDIATELY execute the Skill tool with: bmad-agent-bmm-dev
This invokes the BMAD DEV agent. You MUST do this FIRST before anything else.

Step 2: Once the BMAD agent is loaded, execute the CR (Code Review) command:
CR for story $story_id

Step 3: When the BMAD code review workflow asks 'What should I do with these issues?':
Choose option 2 (Report only - do not fix).

CRITICAL: This is a READ-ONLY audit. Do NOT fix any issues. ONLY report them.

Step 4: After the BMAD workflow completes, end with this AGENT IDENTIFICATION block:

=== AGENT IDENTIFICATION ===
Agent: [Your agent type from the BMAD agent file]
Persona: [Your persona name from the BMAD agent file]
Loaded files:
  - [exact path to each file loaded by BMAD agent]
=== END IDENTIFICATION ===

IMPORTANT: The BMAD agent will load project-context.md and CLAUDE.md as part of its workflow.
Do NOT bypass the BMAD workflow by reading files directly.
"
...
start_epic_review() {
    ...
    while IFS= read -r story_line; do
        ...
        if review_story "$story_id" "$story_number" "$story_status"; then
            ...
            if [[ "$story_status" == "backlog" ]] || ! test_story_file_exists "$story_id"; then
                skipped=$((skipped + 1))
            else
                failed=$((failed + 1))
            fi
...
check_dependencies() {
    ...
    if [[ ! -f "$OUTPUT_DIR/implementation-artifacts/sprint-status.yaml" ]]; then
        echo -e "${RED}Error: sprint-status.yaml not found at $OUTPUT_DIR/implementation-artifacts/sprint-status.yaml${NC}"
        exit 1
    fi
}
