...
async function getLastEmailFor(request, recipient: string, subjectMatch: RegExp) {
  ...
  if (!match) throw new Error(`No email matching subject ${subjectMatch} for ${recipient}`);
  // Fetch full body
...
test("password reset full flow (story_3_4)", async ({ page, request }) => {
  ...
  const email = await getLastEmailFor(request, "alice@example.com", /password reset/i);
  ...
  const linkMatch = email.Text.match(/https?:\/\/[^\s]+\/auth\/reset\?token=[\w-]+/);
