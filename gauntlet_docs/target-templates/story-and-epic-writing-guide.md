...
- The factory commits, pushes, and (optionally) deploys after each story. If a story is purely backend, the operator can't *test* it until later stories provide the UI. By that time, ten more stories have landed and the integration mess is unrecoverable.
...
- The factory's epic-review chain works *across* the stack; reviews look at how stories integrate. Horizontal slices defeat that — Epic 1 is "all backend, no frontend yet" so there's nothing to review.
...
- BMAD epic + story templates that come bundled with the BMAD agents — these guide the *form*; this guide describes the *factory-friendly content*
