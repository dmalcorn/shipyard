...
The server hosts both the public-facing relay/dashboard endpoints (used by the [Command Bridge](../README.md#public-monitoring-dashboard-command-bridge)) and the operator-facing pipeline endpoints (used by clients invoking the agent or the rebuild loop).
...
Send a single instruction to the agent's ReAct loop. Used by the CLI mode and any client wanting one-shot agent interactions.
...
Run the autonomous rebuild loop on a target project. The target must contain BMAD planning artifacts (`epics.md`, `approved-tech-stack.md`) — these are no longer generated by an `/intake` endpoint, only consumed by `/rebuild`.

**Request body:**

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `target_dir` | string | Yes | Path to the target project. Must be outside Shipyard's tree. Relative paths are resolved to absolute. |
...
For typical operator usage, prefer the CLI form `python -m src.main --rebuild <target_dir>` over the HTTP endpoint — the CLI gives live console output and graceful Ctrl-C pause/resume.
